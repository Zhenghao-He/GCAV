...
from configs import alphas, concepts, bottlenecks, target, num_random_exp, is_attack, model_to_run,save_dir, activation_dir, cav_dir, source_dir, mymodel,sess,working_dir,concepts_string, target
...
def plot_results(results, random_counterpart=None, random_concepts=None, num_random_exp=100, min_p_val=0.05, save_path="/p/realai/zhenghao/CAVFusion/analysis/"):
    ...
    # log_path = os.path.join(save_path, f"log_{'attacked' if is_attack else 'original'}_tmp.txt")
    ...
    with open(log_path, "w") as log:
        ...
        # 保存图表
        fig.tight_layout()
        # pic_path = os.path.join(save_path, f"tcav_results_tmp.png")
